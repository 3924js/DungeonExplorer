...
struct Item 
{
    ...
    Item(int _id, std::string _name, ItemType _type, Rarity _rarity, int _value, std::string _desc,int _count)
        : id(_id), name(_name), type(_type), rarity(_rarity), value(_value), desc(_desc), count(1){
    }

    // Default constructor for empty item initialization
    Item() : id(0), name("None"), type(ItemType::Etc), rarity(Rarity::Common), value(0), desc("") ,count(0){}
};